...
export default function App() {
  ...
  const [tx, setTx] = useState({ open: false });

  const [reg, setReg] = useState({
    beneficiary: "", condition: "", domainHint: "general",
    ghostChain: "base", ghostContractAddress: "", ghostAmount: "1", ttlBlocks: 500,
  });
  const [cl, setCl] = useState({
    commitmentId: "", evidenceText: "", evidenceUrls: "", stake: "0.01",
  });

  const address = user?.wallet?.address || wallets?.[0]?.address || null;
  const busy = tx.open && (tx.phase === "signing" || tx.phase === "confirming");
  ...
  const buildContract = useCallback(async () => {
    ...
    const w = wallets?.[0];
    if (w) {
      try { await w.switchChain(4221); } catch { /* embedded wallet is already on it */ }
  ...
  const refresh = useCallback(async (h) => {
    ...
    try {
      ...
      try {
        localStorage.setItem(CACHE_KEY, JSON.stringify({ commitments: nextC, precedents: nextP, treasury: nextT }));
      } catch { /* non-fatal */ }
    } catch (e) {
  ...
  // Scroll reveal. Content is visible by default; the `.js` flag gates the
  // hidden pre-state so non-JS / headless renders never ship blank, and a
  // timeout failsafe reveals anything the observer misses.
  useEffect(() => {
  ...
  // Drive the transaction modal through a write's lifecycle.
  const runTx = useCallback(async (title, fn) => {
    setTx({ open: true, phase: "signing", title, hash: null, message: "Confirm the transaction in your wallet." });
    const onProgress = (p) => setTx((t) => ({
      ...t, ...p,
      message: p.phase === "confirming"
        ? "Broadcast. Validators are reaching consensus on Bradbury."
        : t.message,
    }));
    try {
      await fn(onProgress);
      setTx((t) => ({ ...t, phase: "success", message: "Confirmed on GenLayer Bradbury." }));
      refresh();
    } catch (e) {
      setTx((t) => ({ ...t, phase: "error", message: e?.shortMessage || e?.message || String(e) }));
    }
  }, [refresh]);

  const canRegister =
    contract && address && reg.beneficiary && reg.condition.length >= 8 && reg.ghostContractAddress;
  const canClaim = contract && address && cl.commitmentId && cl.evidenceText;

  const onRegister = () => runTx("Register commitment", (op) => contract.registerCommitment({
    beneficiary: reg.beneficiary, condition: reg.condition, domainHint: reg.domainHint,
    ghostChain: reg.ghostChain, ghostContractAddress: reg.ghostContractAddress,
    ghostAmountWei: genToWei(reg.ghostAmount), ghostTokenAddress: "",
    ghostTimeoutBlockEvm: 0, ttlBlocks: reg.ttlBlocks,
  }, op));

  const onClaim = () => runTx("Submit claim", (op) => contract.submitClaim({
    commitmentId: cl.commitmentId,
    evidenceText: cl.evidenceText,
    evidenceUrls: (cl.evidenceUrls || "").split(",").map((s) => s.trim()).filter((s) => s.startsWith("http")),
  ...
  return (
    <div className="ht-app" id="top">
      ...
      {/* HERO — full-bleed band, asymmetric content */}
      <section className="ht-hero-band">
        <div className="ht-hero-band__bg" aria-hidden />
        <div className="ht-wrap ht-hero">
          <div className="ht-hero__copy">
            <span className="ht-tag ht-hero__pill">{CHAIN.toUpperCase()} testnet · live</span>
            <h1 className="ht-h1">Capital, bound to language.</h1>
      ...
      {/* LIVE PROTOCOL — full-bleed band */}
      <section id="protocol" className="ht-band">
        <div className="ht-wrap">
          <div className="ht-section__head" data-reveal>
            <h2 className="ht-h2">Interact with the live protocol.</h2>
            <p className="ht-lead">
          ...
          <div className="ht-stats" data-reveal>
            {[
              ["Commitments", nC, ""],
              ["Precedents", nP, ""],
              ...
              <Box>
                ...
                <div className="ht-fields">
                  ...
                  <button type="button" className="ht-btn ht-btn--solid" disabled={busy || !canRegister} onClick={onRegister}>
                    <Dots /><span>Register commitment</span>
                  </button>
                </div>
              </Box>
              ...
              <Box>
                ...
                {!loading && commitments.length === 0 ? (
                  <div className="ht-empty">
                    ...
                    <p className="ht-dim">Register one to escrow capital against an obligation.</p>
                  </div>
                ) : (
      ...
      <TxModal tx={tx} onClose={() => setTx({ open: false })} />
